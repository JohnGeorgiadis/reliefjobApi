...
export const getReliefApiJobs = async () => {
    ...
    do {
        ...
        //Writing to DB
        results.data.forEach((jobData: ReliefApiData) => addDocToCollection(normalizeData(jobData), Collections.JOBS));
    } while (endpoint);
...
const normalizeData = (originalData: ReliefApiData) => {
    const fieldsToPopulateKeywords = ['country', 'career_categories', 'city', 'source', 'type', 'experience', 'theme'];
